...
const createReleaseService = ({ strapi }: { strapi: LoadedStrapi }) => ({
  ...
  deleteAction(actionId: DeleteReleaseAction.Request['params']['actionId']) {
    // An action can only belong to a single release
    return strapi.entityService.delete(RELEASE_ACTION_MODEL_UID, actionId);
  },
});
